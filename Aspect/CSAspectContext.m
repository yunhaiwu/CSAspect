...
@property (nonatomic, strong) id<CSAspectDefine> aspectRegisterDefine;
...
+ (instancetype)buildContext:(id<CSAspectDefine>)aspectRegisterDefine {
...
- (instancetype)initWithAspectDefine:(id<CSAspectDefine>)aspectRegisterDefine {
    self = [super init];
    if (self) {
...
- (void)handlePointExpressions {
    ...
    NSString *pointcutExpressions = [[self.aspectRegisterDefine aspectClass] pointcutExpressions];
...
- (NSSet<NSString*>*)pointcutClassNames {
    NSArray *clsList = [_pointcuts allKeys];
    if ([clsList count]) {
...
- (NSSet<NSString*>*)pointcutMethods:(NSString*)className {
    ...
    if ([generalMethods count]) {
        [result addObjectsFromArray:generalMethods.allObjects];
    }
    NSMutableSet *classMethods = _pointcuts[className];
    if ([classMethods count]) {
        [result addObjectsFromArray:classMethods.allObjects];
    }
    return result;
}
